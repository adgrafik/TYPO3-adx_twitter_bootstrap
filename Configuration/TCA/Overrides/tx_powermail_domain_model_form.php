...
if ($extensionConfiguration['extensions.']['activatePowermail'] && \TYPO3\CMS\Core\Utility\ExtensionManagementUtility::isLoaded('powermail')) {
	...
	$GLOBALS['TCA']['tx_powermail_domain_model_form']['palettes']['header'] = array(
		'showitem' => 'title,--linebreak--,tx_adxtwitterbootstrap_header_layout',
		'canNotCollapse' => TRUE,
	);

	$GLOBALS['TCA']['tx_powermail_domain_model_form']['palettes']['layout'] = array(
	...
	\TYPO3\CMS\Core\Utility\ExtensionManagementUtility::addToAllTCAtypes('tx_powermail_domain_model_form', '--palette--;;header', '', 'replace:title');
	\TYPO3\CMS\Core\Utility\ExtensionManagementUtility::addToAllTCAtypes('tx_powermail_domain_model_form', '--palette--;LLL:EXT:adx_twitter_bootstrap/Resources/Private/Language/locallang_db_powermail.xlf:tx_powermail_domain_model_form.palette.apperance;layout', '', 'replace:css');
}
