...
class Page extends \In2code\Powermail\Domain\Model\Page {
	...
	/**
	 * @var \TYPO3\CMS\Extbase\Persistence\ObjectStorage<\AdGrafik\AdxTwitterBootstrap\Domain\Model\Field>
	 * @lazy
	 */
	protected $fields;
	...
	public function getLabelToFieldRatioClass() {
		...
		if (strpos($labelToFieldRatio, ':')) {
			...
			$classes['label'] = sprintf('col-%s-%s', $this->getLabelToFieldRatioBreakpoint(), $labelToFieldRatio[0]);
			$classes['field'] = sprintf('col-%s-%s', $this->getLabelToFieldRatioBreakpoint(), $labelToFieldRatio[1]);
		}
